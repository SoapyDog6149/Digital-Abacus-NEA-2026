...
# Assuming b_str_conv is defined, e.g.:
def b_str_conv(b):
...
def ping_pong_program():
    ...
    main_mac = b'\x98\xa3\x16\xf0\x25\x34'  # Main ESP32's MAC
    # main_mac = b'\xff\xff\xff\xff\xff\xff'  # Broadcast for testing
    ...
    def send_pong(m_msg):  # Send and print
        if e.send(main_mac, m_msg, True):
            print(f"Sent to Main: {m_msg}")
        else:
            print("Failed to send (returned False)")

    while not received_ping:
        try:
            # Listen with timeout (wait up to 2 seconds for message)
            host, main_msg = e.recv(2000)
            print("Listening...")

            if main_msg is not None:
                print(f"Received from {host}: {b_str_conv(main_msg)}")
                n_main_msg = b_str_conv(main_msg)
                if n_main_msg == "main ping":
                    send_pong("cyd pong")
                    espnow_est = True
                    received_ping = True
                    break
            else:
                print("No message received in timeout")
...
versiontext.set_text(f"#5c5c5c VERSION 1.0.0 #")
...
main_logo.set_text(f"#5c5c5c Digital Abacus#")
main_logo.align(lv.ALIGN.CENTER, 0, -12)
...
###
segment_80 = lv.font_load(f"{fs_drive_letter}:fonts/segment_80.bin")
gc.collect()

segment_80_style = lv.style_t()
segment_80_style.init()
segment_80_style.set_text_font(segment_80)
segment_80_style.set_text_color(lv.color_hex(0x5c5c5c))
gc.collect()
...
### ESPNOW listen and sending functions for CYD ###

def listen_msg():
    try:
        host, main_msg = e.recv(2000)  # Listen with a 2-second timeout
        if main_msg is not None:
            host_str = ':'.join(f'{b:02x}' for b in host) if host else 'Unknown'
            msg_str = main_msg.decode('utf-8') if main_msg else ''
            print(f"Received from {host_str}: {msg_str}")
            
            if msg_str == "spin program complete":  # If received confirmation
                return True
...
def send_message(msg):
    try:
        success = e.send(main_mac, msg.encode('utf-8'), True)
        if success:
            print(f"Sent to Main: {msg}")
            return True
        else:
            print("Failed to send (returned False)")
            return False
...
def timer_callback(timer):
    ...
    if remaining_time > 0:
        ...
        menu_btn.align(lv.ALIGN.CENTER, 85, 20)
...
def main_timer():
    global timer_label, timer_object, finish_btn
...
def game_start():
    ...
    lv.scr_load(play_screen)
    
    #Create timer label
    timer_label = lv.label(play_screen)
    timer_label.set_text("--:--") # Initial display
    timer_label.align(lv.ALIGN.CENTER, -80, 0)
    timer_label.add_style(segment_80_style, 0)
    ...
    spin_btn.align(lv.ALIGN.CENTER, 90, 10)
...
def game_play():
    global finish_btn, remaining_time
    print("Game Started...")
    spin_btn.delete()
    
    random_number = random.randint(int(random_int_min), int(random_int_max))
    label_random = lv.label(play_screen) #random number
    label_random.set_recolor(True)
    label_random.add_style(segment_80_style, 0)
    ...
    finish_btn.align(lv.ALIGN.CENTER, 90, 20)
    ...
    def finish_btn_cb(e):
        global timer_complete, timer_object
        print("Timer manual finish")
        timer_object.pause()
        timer_complete = True
        finish_btn.delete() #delete the button
        
        timer_object._del() # Delete the timer when done
        timer_object = None
...
mod_timer_btn.set_size(150, 50)
mod_timer_btn.align(lv.ALIGN.CENTER, 0, -30)
...
mod_min_int_btn.align(lv.ALIGN.CENTER, 0, 40)
...
mod_max_int_btn.align(lv.ALIGN.CENTER, 0, 100)
